...
assert_contains 'For plan, deps, and -S --select; changes presentation only, not execution' "$english_help"
assert_contains '--details' "$japanese_help"
assert_contains '詳細な診断情報と由来情報を表示' "$japanese_help"
assert_contains 'plan、deps、-S --selectで使用可能。表示だけを変更し、実行動作は変えません' "$japanese_help"
