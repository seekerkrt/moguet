...
assert_contains 'For remote build, plan, deps, -S --select, -Qua, -Syu / -Su, upgrade-aur, upgrade-all, --dry-run -S, and list-patch; changes presentation only, not execution' "$english_help"
assert_contains '--details' "$japanese_help"
assert_contains '詳細な診断情報と由来情報を表示' "$japanese_help"
assert_contains 'リモートbuild、plan、deps、-S --select、-Qua、-Syu / -Su、upgrade-aur、upgrade-all、--dry-run -S、list-patchで使用可能。表示だけを変更し、実行動作は変えません' "$japanese_help"
assert_contains 'list-patch' "$english_help"
assert_contains 'List saved patch associations without checking external material' "$english_help"
assert_contains 'list-patch' "$japanese_help"
assert_contains '外部materialを検証せず保存済みpatch associationを一覧' "$japanese_help"
