...
[ "$(LC_ALL=C sort -u "$make_aliases" | wc -l)" -eq "$#" ] ||
...
printf '%s\n' \
    "build-authority-closure-test: Make aliases=$(wc -l < "$make_aliases"), CMake targets=$(wc -l < "$cmake_aliases"), missing=0, unexpected=0"
