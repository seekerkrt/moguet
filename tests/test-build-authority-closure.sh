...
[ "$#" -eq 131 ] ||
    fail "Make focused alias inventory is $#, expected 131"
...
[ "$(LC_ALL=C sort -u "$make_aliases" | wc -l)" -eq 131 ] ||
...
[ "$(wc -l < "$cmake_aliases")" -eq 131 ] ||
    fail "CMake focused target inventory is $(wc -l < "$cmake_aliases"), expected 131"
...
printf '%s\n' \
    'build-authority-closure-test: Make aliases=131, CMake targets=131, missing=0, unexpected=0'
