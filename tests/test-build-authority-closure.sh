...
[ "$#" -eq 129 ] ||
    fail "Make focused alias inventory is $#, expected 129"
...
[ "$(LC_ALL=C sort -u "$make_aliases" | wc -l)" -eq 129 ] ||
...
[ "$(wc -l < "$cmake_aliases")" -eq 129 ] ||
    fail "CMake focused target inventory is $(wc -l < "$cmake_aliases"), expected 129"
...
printf '%s\n' \
    'build-authority-closure-test: Make aliases=129, CMake targets=129, missing=0, unexpected=0'
