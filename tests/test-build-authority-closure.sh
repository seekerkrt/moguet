...
[ "$#" -eq 130 ] ||
    fail "Make focused alias inventory is $#, expected 130"
...
[ "$(LC_ALL=C sort -u "$make_aliases" | wc -l)" -eq 130 ] ||
...
[ "$(wc -l < "$cmake_aliases")" -eq 130 ] ||
    fail "CMake focused target inventory is $(wc -l < "$cmake_aliases"), expected 130"
...
printf '%s\n' \
    'build-authority-closure-test: Make aliases=130, CMake targets=130, missing=0, unexpected=0'
