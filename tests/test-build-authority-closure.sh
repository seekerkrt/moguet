...
[ "$#" -eq 127 ] ||
    fail "Make focused alias inventory is $#, expected 127"
...
[ "$(LC_ALL=C sort -u "$make_aliases" | wc -l)" -eq 127 ] ||
...
[ "$(wc -l < "$cmake_aliases")" -eq 127 ] ||
    fail "CMake focused target inventory is $(wc -l < "$cmake_aliases"), expected 127"
...
printf '%s\n' \
    'build-authority-closure-test: Make aliases=127, CMake targets=127, missing=0, unexpected=0'
