...
[ "$#" -eq 128 ] ||
    fail "Make focused alias inventory is $#, expected 128"
...
[ "$(LC_ALL=C sort -u "$make_aliases" | wc -l)" -eq 128 ] ||
...
[ "$(wc -l < "$cmake_aliases")" -eq 128 ] ||
    fail "CMake focused target inventory is $(wc -l < "$cmake_aliases"), expected 128"
...
printf '%s\n' \
    'build-authority-closure-test: Make aliases=128, CMake targets=128, missing=0, unexpected=0'
