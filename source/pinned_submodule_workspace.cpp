...
std::uintmax_t effective_inventory_byte_limit() {
#ifdef MOGUET_ENABLE_PINNED_SUBMODULE_WORKSPACE_TEST_HOOKS
    return std::min(MAX_BYTES, g_hooks.inventory_byte_limit.value_or(MAX_BYTES));
#else
    return MAX_BYTES;
#endif
}
...
void scan(int root, const fs::path& prefix, dev_t device, std::size_t depth, Stage stage, ScanBudget& budget, Inventory& result, bool mutable_content = false) {
    ...
    while(true) {
        ...
        if(S_ISREG(identity.st_mode)) {
            if(identity.st_size < 0) fail(stage, Reason::ResourceLimitExceeded);
            // Execution-generated bytes are not source authority; every path/stat
            // still participates in the mutable structural and binding proof.
            if(!mutable_content) {
                if(static_cast<std::uintmax_t>(identity.st_size) > effective_inventory_byte_limit() - budget.bytes) fail(stage, Reason::ResourceLimitExceeded);
