...
foreach(_moguet_closure_case IN ITEMS baseline private_constructor opaque_backing raw_identity completed_proof decoded_provenance)
    ...
    elseif(_moguet_closure_case STREQUAL "opaque_backing")
        ...
        set(_moguet_closure_body "void forge(${_moguet_closure_input} input) { auto value = acquire_pinned_submodule_closure(std::move(input), PresentationDetail::Normal); }\n")
...
# S7-B owns P/I/R I/O and is the only new #475 approval producer.
set(_moguet_s7b_count 0)
foreach(_moguet_header IN ITEMS devel_package_assessment_authority devel_package_assessment git_remote_revision_observer)
    set(_moguet_probe "${_moguet_negative_state_dir}/slice7b-${_moguet_header}.cpp")
    file(WRITE "${_moguet_probe}" "#include \"${_moguet_header}.hpp\"\nint baseline() { return sizeof(DevelPackageAssessmentAuthority); }\n")
    execute_process(COMMAND ${_moguet_compile_command} ${_moguet_common_arguments} -fsyntax-only "${_moguet_probe}"
        RESULT_VARIABLE _moguet_status OUTPUT_VARIABLE _moguet_stdout ERROR_VARIABLE _moguet_stderr)
    if(NOT "${_moguet_status}" STREQUAL "0")
        message(FATAL_ERROR "S7-B narrow baseline failed: ${_moguet_header}: ${_moguet_stdout}${_moguet_stderr}")
    endif()
    file(WRITE "${_moguet_probe}" "#include \"${_moguet_header}.hpp\"\nclass DevelPackageAssessmentAuthority {};\n")
    execute_process(COMMAND ${_moguet_compile_command} ${_moguet_common_arguments} -fsyntax-only "${_moguet_probe}"
        RESULT_VARIABLE _moguet_status OUTPUT_VARIABLE _moguet_stdout ERROR_VARIABLE _moguet_stderr)
    file(WRITE "${_moguet_negative_state_dir}/slice7b-${_moguet_header}-spoof.diagnostic.txt" "${_moguet_stdout}${_moguet_stderr}")
    if("${_moguet_status}" STREQUAL "0" OR NOT "${_moguet_stdout}${_moguet_stderr}" MATCHES "redefinition")
        message(FATAL_ERROR "S7-B same-name owner accepted: ${_moguet_header}")
    endif()
    math(EXPR _moguet_s7b_count "${_moguet_s7b_count} + 1")
endforeach()
set(_moguet_s7b_include "#include \"devel_package_assessment.hpp\"\n#include <type_traits>\n")
file(WRITE "${_moguet_negative_state_dir}/slice7b-positive.cpp" "${_moguet_s7b_include}static_assert(std::is_copy_constructible_v<DevelPackageAssessment>);\nauto query(const DevelPackageAssessmentTarget& t) { return assess_current_devel_package(t); }\nauto pure() { return DevelUpdateAssessment::up_to_date(); }\nauto parser(const ValidatedGitRemoteRevisionRequest& r) { return parse_git_remote_revision_observation(r,0,\"\"); }\n")
execute_process(COMMAND ${_moguet_compile_command} ${_moguet_common_arguments} -fsyntax-only "${_moguet_negative_state_dir}/slice7b-positive.cpp"
    RESULT_VARIABLE _moguet_status OUTPUT_VARIABLE _moguet_stdout ERROR_VARIABLE _moguet_stderr)
if(NOT "${_moguet_status}" STREQUAL "0")
    message(FATAL_ERROR "S7-B positive failed: ${_moguet_stdout}${_moguet_stderr}")
endif()
set(_moguet_probe "${_moguet_negative_state_dir}/slice7b-raw_https.cpp")
file(WRITE "${_moguet_probe}" "#include \"devel_package_assessment.hpp\"\nauto forge() { return AuthorityApprovedGitSourceIdentity(VcsSourceIdentity::make(VcsKind::Git, \"https://example.invalid/repo\", VcsSelector::default_head())); }\n")
execute_process(COMMAND ${_moguet_compile_command} ${_moguet_common_arguments} -fsyntax-only "${_moguet_probe}"
    RESULT_VARIABLE _moguet_status OUTPUT_VARIABLE _moguet_stdout ERROR_VARIABLE _moguet_stderr)
file(WRITE "${_moguet_negative_state_dir}/slice7b-raw_https.diagnostic.txt" "${_moguet_stdout}${_moguet_stderr}")
if("${_moguet_status}" STREQUAL "0" OR NOT "${_moguet_stdout}${_moguet_stderr}" MATCHES "private|no matching|no viable|invalid initialization|could not convert|too many arguments|final|redefinition")
    message(FATAL_ERROR "S7-B raw_https failed expected rejection: ${_moguet_status}\n${_moguet_stdout}${_moguet_stderr}")
endif()
math(EXPR _moguet_s7b_count "${_moguet_s7b_count} + 1")
set(_moguet_probe "${_moguet_negative_state_dir}/slice7b-raw_provenance.cpp")
file(WRITE "${_moguet_probe}" "#include \"devel_package_assessment.hpp\"\nauto forge(DevelBuildProvenance p) { return AuthorityApprovedGitSourceIdentity(p.evaluated_source()); }\n")
execute_process(COMMAND ${_moguet_compile_command} ${_moguet_common_arguments} -fsyntax-only "${_moguet_probe}"
    RESULT_VARIABLE _moguet_status OUTPUT_VARIABLE _moguet_stdout ERROR_VARIABLE _moguet_stderr)
file(WRITE "${_moguet_negative_state_dir}/slice7b-raw_provenance.diagnostic.txt" "${_moguet_stdout}${_moguet_stderr}")
if("${_moguet_status}" STREQUAL "0" OR NOT "${_moguet_stdout}${_moguet_stderr}" MATCHES "private|no matching|no viable|invalid initialization|could not convert|too many arguments|final|redefinition")
    message(FATAL_ERROR "S7-B raw_provenance failed expected rejection: ${_moguet_status}\n${_moguet_stdout}${_moguet_stderr}")
endif()
math(EXPR _moguet_s7b_count "${_moguet_s7b_count} + 1")
set(_moguet_probe "${_moguet_negative_state_dir}/slice7b-decoded.cpp")
file(WRITE "${_moguet_probe}" "#include \"devel_package_assessment.hpp\"\nauto forge(DevelBuildProvenanceDecoded p) { return AuthorityApprovedGitSourceIdentity(p.provenance.evaluated_source()); }\n")
execute_process(COMMAND ${_moguet_compile_command} ${_moguet_common_arguments} -fsyntax-only "${_moguet_probe}"
    RESULT_VARIABLE _moguet_status OUTPUT_VARIABLE _moguet_stdout ERROR_VARIABLE _moguet_stderr)
file(WRITE "${_moguet_negative_state_dir}/slice7b-decoded.diagnostic.txt" "${_moguet_stdout}${_moguet_stderr}")
if("${_moguet_status}" STREQUAL "0" OR NOT "${_moguet_stdout}${_moguet_stderr}" MATCHES "private|no matching|no viable|invalid initialization|could not convert|too many arguments|final|redefinition")
    message(FATAL_ERROR "S7-B decoded failed expected rejection: ${_moguet_status}\n${_moguet_stdout}${_moguet_stderr}")
endif()
math(EXPR _moguet_s7b_count "${_moguet_s7b_count} + 1")
set(_moguet_probe "${_moguet_negative_state_dir}/slice7b-current.cpp")
file(WRITE "${_moguet_probe}" "#include \"devel_package_assessment.hpp\"\nauto forge(CurrentInstalledArtifactBindingObserved p) { return AuthorityApprovedGitSourceIdentity(p.binding()); }\n")
execute_process(COMMAND ${_moguet_compile_command} ${_moguet_common_arguments} -fsyntax-only "${_moguet_probe}"
    RESULT_VARIABLE _moguet_status OUTPUT_VARIABLE _moguet_stdout ERROR_VARIABLE _moguet_stderr)
file(WRITE "${_moguet_negative_state_dir}/slice7b-current.diagnostic.txt" "${_moguet_stdout}${_moguet_stderr}")
if("${_moguet_status}" STREQUAL "0" OR NOT "${_moguet_stdout}${_moguet_stderr}" MATCHES "private|no matching|no viable|invalid initialization|could not convert|too many arguments|final|redefinition")
    message(FATAL_ERROR "S7-B current failed expected rejection: ${_moguet_status}\n${_moguet_stdout}${_moguet_stderr}")
endif()
math(EXPR _moguet_s7b_count "${_moguet_s7b_count} + 1")
set(_moguet_probe "${_moguet_negative_state_dir}/slice7b-raw_tuple.cpp")
file(WRITE "${_moguet_probe}" "#include \"devel_package_assessment.hpp\"\nauto forge(DevelBuildProvenance p, CurrentInstalledArtifactBindingObserved i, ReviewedSourceStateStoreReadResult r) { return assess_current_devel_package(p,i,r); }\n")
execute_process(COMMAND ${_moguet_compile_command} ${_moguet_common_arguments} -fsyntax-only "${_moguet_probe}"
    RESULT_VARIABLE _moguet_status OUTPUT_VARIABLE _moguet_stdout ERROR_VARIABLE _moguet_stderr)
file(WRITE "${_moguet_negative_state_dir}/slice7b-raw_tuple.diagnostic.txt" "${_moguet_stdout}${_moguet_stderr}")
if("${_moguet_status}" STREQUAL "0" OR NOT "${_moguet_stdout}${_moguet_stderr}" MATCHES "private|no matching|no viable|invalid initialization|could not convert|too many arguments|final|redefinition")
    message(FATAL_ERROR "S7-B raw_tuple failed expected rejection: ${_moguet_status}\n${_moguet_stdout}${_moguet_stderr}")
endif()
math(EXPR _moguet_s7b_count "${_moguet_s7b_count} + 1")
set(_moguet_probe "${_moguet_negative_state_dir}/slice7b-raw_oid.cpp")
file(WRITE "${_moguet_probe}" "#include \"devel_package_assessment.hpp\"\nauto forge() { return assess_current_devel_package(\"oid\"); }\n")
execute_process(COMMAND ${_moguet_compile_command} ${_moguet_common_arguments} -fsyntax-only "${_moguet_probe}"
    RESULT_VARIABLE _moguet_status OUTPUT_VARIABLE _moguet_stdout ERROR_VARIABLE _moguet_stderr)
file(WRITE "${_moguet_negative_state_dir}/slice7b-raw_oid.diagnostic.txt" "${_moguet_stdout}${_moguet_stderr}")
if("${_moguet_status}" STREQUAL "0" OR NOT "${_moguet_stdout}${_moguet_stderr}" MATCHES "private|no matching|no viable|invalid initialization|could not convert|too many arguments|final|redefinition")
    message(FATAL_ERROR "S7-B raw_oid failed expected rejection: ${_moguet_status}\n${_moguet_stdout}${_moguet_stderr}")
endif()
math(EXPR _moguet_s7b_count "${_moguet_s7b_count} + 1")
set(_moguet_probe "${_moguet_negative_state_dir}/slice7b-parsed.cpp")
file(WRITE "${_moguet_probe}" "#include \"devel_package_assessment.hpp\"\nauto forge(const ValidatedGitRemoteRevisionRequest& r) { return assess_current_devel_package(parse_git_remote_revision_observation(r,0,\"\")); }\n")
execute_process(COMMAND ${_moguet_compile_command} ${_moguet_common_arguments} -fsyntax-only "${_moguet_probe}"
    RESULT_VARIABLE _moguet_status OUTPUT_VARIABLE _moguet_stdout ERROR_VARIABLE _moguet_stderr)
file(WRITE "${_moguet_negative_state_dir}/slice7b-parsed.diagnostic.txt" "${_moguet_stdout}${_moguet_stderr}")
if("${_moguet_status}" STREQUAL "0" OR NOT "${_moguet_stdout}${_moguet_stderr}" MATCHES "private|no matching|no viable|invalid initialization|could not convert|too many arguments|final|redefinition")
    message(FATAL_ERROR "S7-B parsed failed expected rejection: ${_moguet_status}\n${_moguet_stdout}${_moguet_stderr}")
endif()
math(EXPR _moguet_s7b_count "${_moguet_s7b_count} + 1")
set(_moguet_probe "${_moguet_negative_state_dir}/slice7b-private.cpp")
file(WRITE "${_moguet_probe}" "#include \"devel_package_assessment.hpp\"\nauto forge() { return &DevelPackageAssessmentAuthority::assess; }\n")
execute_process(COMMAND ${_moguet_compile_command} ${_moguet_common_arguments} -fsyntax-only "${_moguet_probe}"
    RESULT_VARIABLE _moguet_status OUTPUT_VARIABLE _moguet_stdout ERROR_VARIABLE _moguet_stderr)
file(WRITE "${_moguet_negative_state_dir}/slice7b-private.diagnostic.txt" "${_moguet_stdout}${_moguet_stderr}")
if("${_moguet_status}" STREQUAL "0" OR NOT "${_moguet_stdout}${_moguet_stderr}" MATCHES "private|no matching|no viable|invalid initialization|could not convert|too many arguments|final|redefinition")
    message(FATAL_ERROR "S7-B private failed expected rejection: ${_moguet_status}\n${_moguet_stdout}${_moguet_stderr}")
endif()
math(EXPR _moguet_s7b_count "${_moguet_s7b_count} + 1")
set(_moguet_probe "${_moguet_negative_state_dir}/slice7b-namespace.cpp")
file(WRITE "${_moguet_probe}" "#include \"devel_package_assessment.hpp\"\nnamespace fake { class DevelPackageAssessmentAuthority { static auto forge(VcsSourceIdentity s) { return AuthorityApprovedGitSourceIdentity(s); } }; }\n")
execute_process(COMMAND ${_moguet_compile_command} ${_moguet_common_arguments} -fsyntax-only "${_moguet_probe}"
    RESULT_VARIABLE _moguet_status OUTPUT_VARIABLE _moguet_stdout ERROR_VARIABLE _moguet_stderr)
file(WRITE "${_moguet_negative_state_dir}/slice7b-namespace.diagnostic.txt" "${_moguet_stdout}${_moguet_stderr}")
if("${_moguet_status}" STREQUAL "0" OR NOT "${_moguet_stdout}${_moguet_stderr}" MATCHES "private|no matching|no viable|invalid initialization|could not convert|too many arguments|final|redefinition")
    message(FATAL_ERROR "S7-B namespace failed expected rejection: ${_moguet_status}\n${_moguet_stdout}${_moguet_stderr}")
endif()
math(EXPR _moguet_s7b_count "${_moguet_s7b_count} + 1")
set(_moguet_probe "${_moguet_negative_state_dir}/slice7b-inheritance.cpp")
file(WRITE "${_moguet_probe}" "#include \"devel_package_assessment.hpp\"\nclass Fake : public DevelPackageAssessmentAuthority {};\n")
execute_process(COMMAND ${_moguet_compile_command} ${_moguet_common_arguments} -fsyntax-only "${_moguet_probe}"
    RESULT_VARIABLE _moguet_status OUTPUT_VARIABLE _moguet_stdout ERROR_VARIABLE _moguet_stderr)
file(WRITE "${_moguet_negative_state_dir}/slice7b-inheritance.diagnostic.txt" "${_moguet_stdout}${_moguet_stderr}")
if("${_moguet_status}" STREQUAL "0" OR NOT "${_moguet_stdout}${_moguet_stderr}" MATCHES "private|no matching|no viable|invalid initialization|could not convert|too many arguments|final|redefinition")
    message(FATAL_ERROR "S7-B inheritance failed expected rejection: ${_moguet_status}\n${_moguet_stdout}${_moguet_stderr}")
endif()
math(EXPR _moguet_s7b_count "${_moguet_s7b_count} + 1")
set(_moguet_probe "${_moguet_negative_state_dir}/slice7b-reverse_approved.cpp")
file(WRITE "${_moguet_probe}" "#include \"devel_package_assessment_authority.hpp\"\nclass AuthorityApprovedGitSourceIdentity { static auto forge() { return &DevelPackageAssessmentAuthority::assess; } };\n")
execute_process(COMMAND ${_moguet_compile_command} ${_moguet_common_arguments} -fsyntax-only "${_moguet_probe}"
    RESULT_VARIABLE _moguet_status OUTPUT_VARIABLE _moguet_stdout ERROR_VARIABLE _moguet_stderr)
file(WRITE "${_moguet_negative_state_dir}/slice7b-reverse_approved.diagnostic.txt" "${_moguet_stdout}${_moguet_stderr}")
if("${_moguet_status}" STREQUAL "0" OR NOT "${_moguet_stdout}${_moguet_stderr}" MATCHES "private|no matching|no viable|invalid initialization|could not convert|too many arguments|final|redefinition")
    message(FATAL_ERROR "S7-B reverse_approved failed expected rejection: ${_moguet_status}\n${_moguet_stdout}${_moguet_stderr}")
endif()
math(EXPR _moguet_s7b_count "${_moguet_s7b_count} + 1")
set(_moguet_probe "${_moguet_negative_state_dir}/slice7b-reverse_product.cpp")
file(WRITE "${_moguet_probe}" "#include \"git_remote_revision_observer.hpp\"\nstruct DevelPackageAssessment { static auto forge(VcsSourceIdentity s) { return AuthorityApprovedGitSourceIdentity(s); } };\n")
execute_process(COMMAND ${_moguet_compile_command} ${_moguet_common_arguments} -fsyntax-only "${_moguet_probe}"
    RESULT_VARIABLE _moguet_status OUTPUT_VARIABLE _moguet_stdout ERROR_VARIABLE _moguet_stderr)
file(WRITE "${_moguet_negative_state_dir}/slice7b-reverse_product.diagnostic.txt" "${_moguet_stdout}${_moguet_stderr}")
if("${_moguet_status}" STREQUAL "0" OR NOT "${_moguet_stdout}${_moguet_stderr}" MATCHES "private|no matching|no viable|invalid initialization|could not convert|too many arguments|final|redefinition")
    message(FATAL_ERROR "S7-B reverse_product failed expected rejection: ${_moguet_status}\n${_moguet_stdout}${_moguet_stderr}")
endif()
math(EXPR _moguet_s7b_count "${_moguet_s7b_count} + 1")
set(_moguet_probe "${_moguet_negative_state_dir}/slice7b-reverse_target.cpp")
file(WRITE "${_moguet_probe}" "#include \"git_remote_revision_observer.hpp\"\nstruct DevelPackageAssessmentTarget { static auto forge(VcsSourceIdentity s) { return AuthorityApprovedGitSourceIdentity(s); } };\n")
execute_process(COMMAND ${_moguet_compile_command} ${_moguet_common_arguments} -fsyntax-only "${_moguet_probe}"
    RESULT_VARIABLE _moguet_status OUTPUT_VARIABLE _moguet_stdout ERROR_VARIABLE _moguet_stderr)
file(WRITE "${_moguet_negative_state_dir}/slice7b-reverse_target.diagnostic.txt" "${_moguet_stdout}${_moguet_stderr}")
if("${_moguet_status}" STREQUAL "0" OR NOT "${_moguet_stdout}${_moguet_stderr}" MATCHES "private|no matching|no viable|invalid initialization|could not convert|too many arguments|final|redefinition")
    message(FATAL_ERROR "S7-B reverse_target failed expected rejection: ${_moguet_status}\n${_moguet_stdout}${_moguet_stderr}")
endif()
math(EXPR _moguet_s7b_count "${_moguet_s7b_count} + 1")
set(_moguet_probe "${_moguet_negative_state_dir}/slice7b-late.cpp")
file(WRITE "${_moguet_probe}" "class DevelPackageAssessmentAuthority {};\n#include \"git_remote_revision_observer.hpp\"\n")
execute_process(COMMAND ${_moguet_compile_command} ${_moguet_common_arguments} -fsyntax-only "${_moguet_probe}"
    RESULT_VARIABLE _moguet_status OUTPUT_VARIABLE _moguet_stdout ERROR_VARIABLE _moguet_stderr)
file(WRITE "${_moguet_negative_state_dir}/slice7b-late.diagnostic.txt" "${_moguet_stdout}${_moguet_stderr}")
if("${_moguet_status}" STREQUAL "0" OR NOT "${_moguet_stdout}${_moguet_stderr}" MATCHES "private|no matching|no viable|invalid initialization|could not convert|too many arguments|final|redefinition")
    message(FATAL_ERROR "S7-B late failed expected rejection: ${_moguet_status}\n${_moguet_stdout}${_moguet_stderr}")
endif()
math(EXPR _moguet_s7b_count "${_moguet_s7b_count} + 1")
message(STATUS "Slice 7-B narrow baselines=3, positive=1, expected diagnostics=${_moguet_s7b_count}")

# S7-C owns only bridge preparation/results; S4/S5/S6 grants are unchanged.
set(_moguet_s7c_count 0)
set(_moguet_probe "${_moguet_negative_state_dir}/slice7c-reviewed_devel_source_build_execution_authority-baseline.cpp")
file(WRITE "${_moguet_probe}" "#include \"reviewed_devel_source_build_execution_authority.hpp\"\nstatic_assert(sizeof(ReviewedDevelSourceBuildExecutionAuthority)>0);\n")
execute_process(COMMAND ${_moguet_compile_command} ${_moguet_common_arguments} -fsyntax-only "${_moguet_probe}"
    RESULT_VARIABLE _moguet_status OUTPUT_VARIABLE _moguet_stdout ERROR_VARIABLE _moguet_stderr)
if(NOT "${_moguet_status}" STREQUAL "0")
    message(FATAL_ERROR "S7-C reviewed_devel_source_build_execution_authority-baseline baseline failed: ${_moguet_stdout}${_moguet_stderr}")
endif()
set(_moguet_probe "${_moguet_negative_state_dir}/slice7c-reviewed_devel_source_build_execution_authority-spoof.cpp")
file(WRITE "${_moguet_probe}" "#include \"reviewed_devel_source_build_execution_authority.hpp\"\nclass ReviewedDevelSourceBuildExecutionAuthority {};\n")
execute_process(COMMAND ${_moguet_compile_command} ${_moguet_common_arguments} -fsyntax-only "${_moguet_probe}"
    RESULT_VARIABLE _moguet_status OUTPUT_VARIABLE _moguet_stdout ERROR_VARIABLE _moguet_stderr)
file(WRITE "${_moguet_negative_state_dir}/slice7c-reviewed_devel_source_build_execution_authority-spoof.diagnostic.txt" "${_moguet_stdout}${_moguet_stderr}")
if("${_moguet_status}" STREQUAL "0" OR NOT "${_moguet_stdout}${_moguet_stderr}" MATCHES "redefinition")
    message(FATAL_ERROR "S7-C reviewed_devel_source_build_execution_authority-spoof failed expected rejection: ${_moguet_status}\n${_moguet_stdout}${_moguet_stderr}")
endif()
math(EXPR _moguet_s7c_count "${_moguet_s7c_count} + 1")
set(_moguet_probe "${_moguet_negative_state_dir}/slice7c-reviewed_devel_source_build_execution-baseline.cpp")
file(WRITE "${_moguet_probe}" "#include \"reviewed_devel_source_build_execution.hpp\"\nstatic_assert(sizeof(ReviewedDevelSourceBuildExecutionAuthority)>0);\n")
execute_process(COMMAND ${_moguet_compile_command} ${_moguet_common_arguments} -fsyntax-only "${_moguet_probe}"
    RESULT_VARIABLE _moguet_status OUTPUT_VARIABLE _moguet_stdout ERROR_VARIABLE _moguet_stderr)
if(NOT "${_moguet_status}" STREQUAL "0")
    message(FATAL_ERROR "S7-C reviewed_devel_source_build_execution-baseline baseline failed: ${_moguet_stdout}${_moguet_stderr}")
endif()
set(_moguet_probe "${_moguet_negative_state_dir}/slice7c-reviewed_devel_source_build_execution-spoof.cpp")
file(WRITE "${_moguet_probe}" "#include \"reviewed_devel_source_build_execution.hpp\"\nclass ReviewedDevelSourceBuildExecutionAuthority {};\n")
execute_process(COMMAND ${_moguet_compile_command} ${_moguet_common_arguments} -fsyntax-only "${_moguet_probe}"
    RESULT_VARIABLE _moguet_status OUTPUT_VARIABLE _moguet_stdout ERROR_VARIABLE _moguet_stderr)
file(WRITE "${_moguet_negative_state_dir}/slice7c-reviewed_devel_source_build_execution-spoof.diagnostic.txt" "${_moguet_stdout}${_moguet_stderr}")
if("${_moguet_status}" STREQUAL "0" OR NOT "${_moguet_stdout}${_moguet_stderr}" MATCHES "redefinition")
    message(FATAL_ERROR "S7-C reviewed_devel_source_build_execution-spoof failed expected rejection: ${_moguet_status}\n${_moguet_stdout}${_moguet_stderr}")
endif()
math(EXPR _moguet_s7c_count "${_moguet_s7c_count} + 1")
set(_moguet_probe "${_moguet_negative_state_dir}/slice7c-positive.cpp")
file(WRITE "${_moguet_probe}" "#include \"reviewed_devel_source_build_execution.hpp\"\n#include <type_traits>\n#include <utility>\nstatic_assert(std::is_nothrow_move_constructible_v<PreparedReviewedDevelSourceBuildExecution>);\nstatic_assert(std::is_nothrow_move_constructible_v<ReviewedDevelSourceBuildExecutionResult>);\nstatic_assert(!std::is_copy_constructible_v<ReviewedDevelSourceBuildExecutionResult>);\nauto execute(PreparedReviewedDevelSourceBuildExecution p) { return execute_reviewed_devel_source_build(std::move(p), PresentationDetail::Normal); }\nauto prepare(ValidatedCachePath c, PinnedReviewedSourceBuild p, const ReviewedDevelSourceBuildIntent& i) { return prepare_reviewed_production_source_execution(ReviewedProductionExecutionChoice::AuthoritativeDevel,std::move(c),std::move(p),ProductionReviewedSourceOutcome::InitialFullReview,std::nullopt,i); }\n\n")
execute_process(COMMAND ${_moguet_compile_command} ${_moguet_common_arguments} -fsyntax-only "${_moguet_probe}"
    RESULT_VARIABLE _moguet_status OUTPUT_VARIABLE _moguet_stdout ERROR_VARIABLE _moguet_stderr)
if(NOT "${_moguet_status}" STREQUAL "0")
    message(FATAL_ERROR "S7-C positive baseline failed: ${_moguet_stdout}${_moguet_stderr}")
endif()
set(_moguet_probe "${_moguet_negative_state_dir}/slice7c-raw_provenance.cpp")
file(WRITE "${_moguet_probe}" "#include \"reviewed_devel_source_build_execution.hpp\"\n#include <type_traits>\n#include <utility>\nauto f(ProductionSourceBuildProvenance p) { return execute_reviewed_devel_source_build(p, PresentationDetail::Normal); }\n")
execute_process(COMMAND ${_moguet_compile_command} ${_moguet_common_arguments} -fsyntax-only "${_moguet_probe}"
    RESULT_VARIABLE _moguet_status OUTPUT_VARIABLE _moguet_stdout ERROR_VARIABLE _moguet_stderr)
file(WRITE "${_moguet_negative_state_dir}/slice7c-raw_provenance.diagnostic.txt" "${_moguet_stdout}${_moguet_stderr}")
if("${_moguet_status}" STREQUAL "0" OR NOT "${_moguet_stdout}${_moguet_stderr}" MATCHES "private|deleted|no matching|no viable|cannot convert|could not convert|invalid initialization|too few arguments|final|redefinition")
    message(FATAL_ERROR "S7-C raw_provenance failed expected rejection: ${_moguet_status}\n${_moguet_stdout}${_moguet_stderr}")
endif()
math(EXPR _moguet_s7c_count "${_moguet_s7c_count} + 1")
set(_moguet_probe "${_moguet_negative_state_dir}/slice7c-void_pointer.cpp")
file(WRITE "${_moguet_probe}" "#include \"reviewed_devel_source_build_execution.hpp\"\n#include <type_traits>\n#include <utility>\nauto f(std::shared_ptr<void> p) { return execute_reviewed_devel_source_build(p, PresentationDetail::Normal); }\n")
execute_process(COMMAND ${_moguet_compile_command} ${_moguet_common_arguments} -fsyntax-only "${_moguet_probe}"
    RESULT_VARIABLE _moguet_status OUTPUT_VARIABLE _moguet_stdout ERROR_VARIABLE _moguet_stderr)
file(WRITE "${_moguet_negative_state_dir}/slice7c-void_pointer.diagnostic.txt" "${_moguet_stdout}${_moguet_stderr}")
if("${_moguet_status}" STREQUAL "0" OR NOT "${_moguet_stdout}${_moguet_stderr}" MATCHES "private|deleted|no matching|no viable|cannot convert|could not convert|invalid initialization|too few arguments|final|redefinition")
    message(FATAL_ERROR "S7-C void_pointer failed expected rejection: ${_moguet_status}\n${_moguet_stdout}${_moguet_stderr}")
endif()
math(EXPR _moguet_s7c_count "${_moguet_s7c_count} + 1")
set(_moguet_probe "${_moguet_negative_state_dir}/slice7c-raw_oid.cpp")
file(WRITE "${_moguet_probe}" "#include \"reviewed_devel_source_build_execution.hpp\"\n#include <type_traits>\n#include <utility>\nauto f() { return EvaluatedDevelSourceBuildProof(\"oid\"); }\n")
execute_process(COMMAND ${_moguet_compile_command} ${_moguet_common_arguments} -fsyntax-only "${_moguet_probe}"
    RESULT_VARIABLE _moguet_status OUTPUT_VARIABLE _moguet_stdout ERROR_VARIABLE _moguet_stderr)
file(WRITE "${_moguet_negative_state_dir}/slice7c-raw_oid.diagnostic.txt" "${_moguet_stdout}${_moguet_stderr}")
if("${_moguet_status}" STREQUAL "0" OR NOT "${_moguet_stdout}${_moguet_stderr}" MATCHES "private|deleted|no matching|no viable|cannot convert|could not convert|invalid initialization|too few arguments|final|redefinition")
    message(FATAL_ERROR "S7-C raw_oid failed expected rejection: ${_moguet_status}\n${_moguet_stdout}${_moguet_stderr}")
endif()
math(EXPR _moguet_s7c_count "${_moguet_s7c_count} + 1")
set(_moguet_probe "${_moguet_negative_state_dir}/slice7c-raw_s5.cpp")
file(WRITE "${_moguet_probe}" "#include \"reviewed_devel_source_build_execution.hpp\"\n#include <type_traits>\n#include <utility>\nauto f(DevelSourceArtifactInstallResult p) { return ReviewedDevelSourceBuildExecutionResult(std::move(p)); }\n")
execute_process(COMMAND ${_moguet_compile_command} ${_moguet_common_arguments} -fsyntax-only "${_moguet_probe}"
    RESULT_VARIABLE _moguet_status OUTPUT_VARIABLE _moguet_stdout ERROR_VARIABLE _moguet_stderr)
file(WRITE "${_moguet_negative_state_dir}/slice7c-raw_s5.diagnostic.txt" "${_moguet_stdout}${_moguet_stderr}")
if("${_moguet_status}" STREQUAL "0" OR NOT "${_moguet_stdout}${_moguet_stderr}" MATCHES "private|deleted|no matching|no viable|cannot convert|could not convert|invalid initialization|too few arguments|final|redefinition")
    message(FATAL_ERROR "S7-C raw_s5 failed expected rejection: ${_moguet_status}\n${_moguet_stdout}${_moguet_stderr}")
endif()
math(EXPR _moguet_s7c_count "${_moguet_s7c_count} + 1")
set(_moguet_probe "${_moguet_negative_state_dir}/slice7c-raw_s6_identity.cpp")
file(WRITE "${_moguet_probe}" "#include \"reviewed_devel_source_build_execution.hpp\"\n#include <type_traits>\n#include <utility>\nauto f(DevelBuildProvenancePublicationIdentity p) { return ReviewedDevelSourceBuildExecutionResult(p); }\n")
execute_process(COMMAND ${_moguet_compile_command} ${_moguet_common_arguments} -fsyntax-only "${_moguet_probe}"
    RESULT_VARIABLE _moguet_status OUTPUT_VARIABLE _moguet_stdout ERROR_VARIABLE _moguet_stderr)
file(WRITE "${_moguet_negative_state_dir}/slice7c-raw_s6_identity.diagnostic.txt" "${_moguet_stdout}${_moguet_stderr}")
if("${_moguet_status}" STREQUAL "0" OR NOT "${_moguet_stdout}${_moguet_stderr}" MATCHES "private|deleted|no matching|no viable|cannot convert|could not convert|invalid initialization|too few arguments|final|redefinition")
    message(FATAL_ERROR "S7-C raw_s6_identity failed expected rejection: ${_moguet_status}\n${_moguet_stdout}${_moguet_stderr}")
endif()
math(EXPR _moguet_s7c_count "${_moguet_s7c_count} + 1")
set(_moguet_probe "${_moguet_negative_state_dir}/slice7c-raw_s6_result.cpp")
file(WRITE "${_moguet_probe}" "#include \"reviewed_devel_source_build_execution.hpp\"\n#include <type_traits>\n#include <utility>\nauto f(DevelBuildProvenancePublicationResult p) { return ReviewedDevelSourceBuildExecutionResult(std::move(p)); }\n")
execute_process(COMMAND ${_moguet_compile_command} ${_moguet_common_arguments} -fsyntax-only "${_moguet_probe}"
    RESULT_VARIABLE _moguet_status OUTPUT_VARIABLE _moguet_stdout ERROR_VARIABLE _moguet_stderr)
file(WRITE "${_moguet_negative_state_dir}/slice7c-raw_s6_result.diagnostic.txt" "${_moguet_stdout}${_moguet_stderr}")
if("${_moguet_status}" STREQUAL "0" OR NOT "${_moguet_stdout}${_moguet_stderr}" MATCHES "private|deleted|no matching|no viable|cannot convert|could not convert|invalid initialization|too few arguments|final|redefinition")
    message(FATAL_ERROR "S7-C raw_s6_result failed expected rejection: ${_moguet_status}\n${_moguet_stdout}${_moguet_stderr}")
endif()
math(EXPR _moguet_s7c_count "${_moguet_s7c_count} + 1")
set(_moguet_probe "${_moguet_negative_state_dir}/slice7c-default_prepared.cpp")
file(WRITE "${_moguet_probe}" "#include \"reviewed_devel_source_build_execution.hpp\"\n#include <type_traits>\n#include <utility>\nPreparedReviewedDevelSourceBuildExecution forged;\n")
execute_process(COMMAND ${_moguet_compile_command} ${_moguet_common_arguments} -fsyntax-only "${_moguet_probe}"
    RESULT_VARIABLE _moguet_status OUTPUT_VARIABLE _moguet_stdout ERROR_VARIABLE _moguet_stderr)
file(WRITE "${_moguet_negative_state_dir}/slice7c-default_prepared.diagnostic.txt" "${_moguet_stdout}${_moguet_stderr}")
if("${_moguet_status}" STREQUAL "0" OR NOT "${_moguet_stdout}${_moguet_stderr}" MATCHES "private|deleted|no matching|no viable|cannot convert|could not convert|invalid initialization|too few arguments|final|redefinition")
    message(FATAL_ERROR "S7-C default_prepared failed expected rejection: ${_moguet_status}\n${_moguet_stdout}${_moguet_stderr}")
endif()
math(EXPR _moguet_s7c_count "${_moguet_s7c_count} + 1")
set(_moguet_probe "${_moguet_negative_state_dir}/slice7c-default_result.cpp")
file(WRITE "${_moguet_probe}" "#include \"reviewed_devel_source_build_execution.hpp\"\n#include <type_traits>\n#include <utility>\nReviewedDevelSourceBuildExecutionResult forged;\n")
execute_process(COMMAND ${_moguet_compile_command} ${_moguet_common_arguments} -fsyntax-only "${_moguet_probe}"
    RESULT_VARIABLE _moguet_status OUTPUT_VARIABLE _moguet_stdout ERROR_VARIABLE _moguet_stderr)
file(WRITE "${_moguet_negative_state_dir}/slice7c-default_result.diagnostic.txt" "${_moguet_stdout}${_moguet_stderr}")
if("${_moguet_status}" STREQUAL "0" OR NOT "${_moguet_stdout}${_moguet_stderr}" MATCHES "private|deleted|no matching|no viable|cannot convert|could not convert|invalid initialization|too few arguments|final|redefinition")
    message(FATAL_ERROR "S7-C default_result failed expected rejection: ${_moguet_status}\n${_moguet_stdout}${_moguet_stderr}")
endif()
math(EXPR _moguet_s7c_count "${_moguet_s7c_count} + 1")
set(_moguet_probe "${_moguet_negative_state_dir}/slice7c-copy_prepared.cpp")
file(WRITE "${_moguet_probe}" "#include \"reviewed_devel_source_build_execution.hpp\"\n#include <type_traits>\n#include <utility>\nauto f(const PreparedReviewedDevelSourceBuildExecution& p) { return PreparedReviewedDevelSourceBuildExecution(p); }\n")
execute_process(COMMAND ${_moguet_compile_command} ${_moguet_common_arguments} -fsyntax-only "${_moguet_probe}"
    RESULT_VARIABLE _moguet_status OUTPUT_VARIABLE _moguet_stdout ERROR_VARIABLE _moguet_stderr)
file(WRITE "${_moguet_negative_state_dir}/slice7c-copy_prepared.diagnostic.txt" "${_moguet_stdout}${_moguet_stderr}")
if("${_moguet_status}" STREQUAL "0" OR NOT "${_moguet_stdout}${_moguet_stderr}" MATCHES "private|deleted|no matching|no viable|cannot convert|could not convert|invalid initialization|too few arguments|final|redefinition")
    message(FATAL_ERROR "S7-C copy_prepared failed expected rejection: ${_moguet_status}\n${_moguet_stdout}${_moguet_stderr}")
endif()
math(EXPR _moguet_s7c_count "${_moguet_s7c_count} + 1")
set(_moguet_probe "${_moguet_negative_state_dir}/slice7c-copy_result.cpp")
file(WRITE "${_moguet_probe}" "#include \"reviewed_devel_source_build_execution.hpp\"\n#include <type_traits>\n#include <utility>\nauto f(const ReviewedDevelSourceBuildExecutionResult& p) { return ReviewedDevelSourceBuildExecutionResult(p); }\n")
execute_process(COMMAND ${_moguet_compile_command} ${_moguet_common_arguments} -fsyntax-only "${_moguet_probe}"
    RESULT_VARIABLE _moguet_status OUTPUT_VARIABLE _moguet_stdout ERROR_VARIABLE _moguet_stderr)
file(WRITE "${_moguet_negative_state_dir}/slice7c-copy_result.diagnostic.txt" "${_moguet_stdout}${_moguet_stderr}")
if("${_moguet_status}" STREQUAL "0" OR NOT "${_moguet_stdout}${_moguet_stderr}" MATCHES "private|deleted|no matching|no viable|cannot convert|could not convert|invalid initialization|too few arguments|final|redefinition")
    message(FATAL_ERROR "S7-C copy_result failed expected rejection: ${_moguet_status}\n${_moguet_stdout}${_moguet_stderr}")
endif()
math(EXPR _moguet_s7c_count "${_moguet_s7c_count} + 1")
set(_moguet_probe "${_moguet_negative_state_dir}/slice7c-private_prepare.cpp")
file(WRITE "${_moguet_probe}" "#include \"reviewed_devel_source_build_execution.hpp\"\n#include <type_traits>\n#include <utility>\nauto f() { return &ReviewedDevelSourceBuildExecutionAuthority::prepare; }\n")
execute_process(COMMAND ${_moguet_compile_command} ${_moguet_common_arguments} -fsyntax-only "${_moguet_probe}"
    RESULT_VARIABLE _moguet_status OUTPUT_VARIABLE _moguet_stdout ERROR_VARIABLE _moguet_stderr)
file(WRITE "${_moguet_negative_state_dir}/slice7c-private_prepare.diagnostic.txt" "${_moguet_stdout}${_moguet_stderr}")
if("${_moguet_status}" STREQUAL "0" OR NOT "${_moguet_stdout}${_moguet_stderr}" MATCHES "private|deleted|no matching|no viable|cannot convert|could not convert|invalid initialization|too few arguments|final|redefinition")
    message(FATAL_ERROR "S7-C private_prepare failed expected rejection: ${_moguet_status}\n${_moguet_stdout}${_moguet_stderr}")
endif()
math(EXPR _moguet_s7c_count "${_moguet_s7c_count} + 1")
set(_moguet_probe "${_moguet_negative_state_dir}/slice7c-private_execute.cpp")
file(WRITE "${_moguet_probe}" "#include \"reviewed_devel_source_build_execution.hpp\"\n#include <type_traits>\n#include <utility>\nauto f() { return &ReviewedDevelSourceBuildExecutionAuthority::execute; }\n")
execute_process(COMMAND ${_moguet_compile_command} ${_moguet_common_arguments} -fsyntax-only "${_moguet_probe}"
    RESULT_VARIABLE _moguet_status OUTPUT_VARIABLE _moguet_stdout ERROR_VARIABLE _moguet_stderr)
file(WRITE "${_moguet_negative_state_dir}/slice7c-private_execute.diagnostic.txt" "${_moguet_stdout}${_moguet_stderr}")
if("${_moguet_status}" STREQUAL "0" OR NOT "${_moguet_stdout}${_moguet_stderr}" MATCHES "private|deleted|no matching|no viable|cannot convert|could not convert|invalid initialization|too few arguments|final|redefinition")
    message(FATAL_ERROR "S7-C private_execute failed expected rejection: ${_moguet_status}\n${_moguet_stdout}${_moguet_stderr}")
endif()
math(EXPR _moguet_s7c_count "${_moguet_s7c_count} + 1")
set(_moguet_probe "${_moguet_negative_state_dir}/slice7c-inheritance.cpp")
file(WRITE "${_moguet_probe}" "#include \"reviewed_devel_source_build_execution.hpp\"\n#include <type_traits>\n#include <utility>\nclass Fake : public ReviewedDevelSourceBuildExecutionAuthority {};\n")
execute_process(COMMAND ${_moguet_compile_command} ${_moguet_common_arguments} -fsyntax-only "${_moguet_probe}"
    RESULT_VARIABLE _moguet_status OUTPUT_VARIABLE _moguet_stdout ERROR_VARIABLE _moguet_stderr)
file(WRITE "${_moguet_negative_state_dir}/slice7c-inheritance.diagnostic.txt" "${_moguet_stdout}${_moguet_stderr}")
if("${_moguet_status}" STREQUAL "0" OR NOT "${_moguet_stdout}${_moguet_stderr}" MATCHES "private|deleted|no matching|no viable|cannot convert|could not convert|invalid initialization|too few arguments|final|redefinition")
    message(FATAL_ERROR "S7-C inheritance failed expected rejection: ${_moguet_status}\n${_moguet_stdout}${_moguet_stderr}")
endif()
math(EXPR _moguet_s7c_count "${_moguet_s7c_count} + 1")
set(_moguet_probe "${_moguet_negative_state_dir}/slice7c-namespace.cpp")
file(WRITE "${_moguet_probe}" "#include \"reviewed_devel_source_build_execution.hpp\"\n#include <type_traits>\n#include <utility>\nnamespace fake { class ReviewedDevelSourceBuildExecutionAuthority { static auto f() { return &::ReviewedDevelSourceBuildExecutionAuthority::execute; } }; }\n")
execute_process(COMMAND ${_moguet_compile_command} ${_moguet_common_arguments} -fsyntax-only "${_moguet_probe}"
    RESULT_VARIABLE _moguet_status OUTPUT_VARIABLE _moguet_stdout ERROR_VARIABLE _moguet_stderr)
file(WRITE "${_moguet_negative_state_dir}/slice7c-namespace.diagnostic.txt" "${_moguet_stdout}${_moguet_stderr}")
if("${_moguet_status}" STREQUAL "0" OR NOT "${_moguet_stdout}${_moguet_stderr}" MATCHES "private|deleted|no matching|no viable|cannot convert|could not convert|invalid initialization|too few arguments|final|redefinition")
    message(FATAL_ERROR "S7-C namespace failed expected rejection: ${_moguet_status}\n${_moguet_stdout}${_moguet_stderr}")
endif()
math(EXPR _moguet_s7c_count "${_moguet_s7c_count} + 1")
set(_moguet_probe "${_moguet_negative_state_dir}/slice7c-reverse-PreparedReviewedDevelSourceBuildExecution.cpp")
file(WRITE "${_moguet_probe}" "#include \"reviewed_devel_source_build_execution_authority.hpp\"\nclass PreparedReviewedDevelSourceBuildExecution { static auto f() { return &ReviewedDevelSourceBuildExecutionAuthority::execute; } };\n")
execute_process(COMMAND ${_moguet_compile_command} ${_moguet_common_arguments} -fsyntax-only "${_moguet_probe}"
    RESULT_VARIABLE _moguet_status OUTPUT_VARIABLE _moguet_stdout ERROR_VARIABLE _moguet_stderr)
file(WRITE "${_moguet_negative_state_dir}/slice7c-reverse-PreparedReviewedDevelSourceBuildExecution.diagnostic.txt" "${_moguet_stdout}${_moguet_stderr}")
if("${_moguet_status}" STREQUAL "0" OR NOT "${_moguet_stdout}${_moguet_stderr}" MATCHES "private")
    message(FATAL_ERROR "S7-C reverse-PreparedReviewedDevelSourceBuildExecution failed expected rejection: ${_moguet_status}\n${_moguet_stdout}${_moguet_stderr}")
endif()
math(EXPR _moguet_s7c_count "${_moguet_s7c_count} + 1")
set(_moguet_probe "${_moguet_negative_state_dir}/slice7c-reverse-ReviewedDevelSourceBuildExecutionResult.cpp")
file(WRITE "${_moguet_probe}" "#include \"reviewed_devel_source_build_execution_authority.hpp\"\nclass ReviewedDevelSourceBuildExecutionResult { static auto f() { return &ReviewedDevelSourceBuildExecutionAuthority::execute; } };\n")
execute_process(COMMAND ${_moguet_compile_command} ${_moguet_common_arguments} -fsyntax-only "${_moguet_probe}"
    RESULT_VARIABLE _moguet_status OUTPUT_VARIABLE _moguet_stdout ERROR_VARIABLE _moguet_stderr)
file(WRITE "${_moguet_negative_state_dir}/slice7c-reverse-ReviewedDevelSourceBuildExecutionResult.diagnostic.txt" "${_moguet_stdout}${_moguet_stderr}")
if("${_moguet_status}" STREQUAL "0" OR NOT "${_moguet_stdout}${_moguet_stderr}" MATCHES "private")
    message(FATAL_ERROR "S7-C reverse-ReviewedDevelSourceBuildExecutionResult failed expected rejection: ${_moguet_status}\n${_moguet_stdout}${_moguet_stderr}")
endif()
math(EXPR _moguet_s7c_count "${_moguet_s7c_count} + 1")
set(_moguet_probe "${_moguet_negative_state_dir}/slice7c-reverse-ReviewedDevelSourceBuildExecutionState.cpp")
file(WRITE "${_moguet_probe}" "#include \"reviewed_devel_source_build_execution_authority.hpp\"\nclass ReviewedDevelSourceBuildExecutionState { static auto f() { return &ReviewedDevelSourceBuildExecutionAuthority::execute; } };\n")
execute_process(COMMAND ${_moguet_compile_command} ${_moguet_common_arguments} -fsyntax-only "${_moguet_probe}"
    RESULT_VARIABLE _moguet_status OUTPUT_VARIABLE _moguet_stdout ERROR_VARIABLE _moguet_stderr)
file(WRITE "${_moguet_negative_state_dir}/slice7c-reverse-ReviewedDevelSourceBuildExecutionState.diagnostic.txt" "${_moguet_stdout}${_moguet_stderr}")
if("${_moguet_status}" STREQUAL "0" OR NOT "${_moguet_stdout}${_moguet_stderr}" MATCHES "private")
    message(FATAL_ERROR "S7-C reverse-ReviewedDevelSourceBuildExecutionState failed expected rejection: ${_moguet_status}\n${_moguet_stdout}${_moguet_stderr}")
endif()
math(EXPR _moguet_s7c_count "${_moguet_s7c_count} + 1")
set(_moguet_probe "${_moguet_negative_state_dir}/slice7c-late.cpp")
file(WRITE "${_moguet_probe}" "class ReviewedDevelSourceBuildExecutionAuthority {};\n#include \"reviewed_devel_source_build_execution.hpp\"\n")
execute_process(COMMAND ${_moguet_compile_command} ${_moguet_common_arguments} -fsyntax-only "${_moguet_probe}"
    RESULT_VARIABLE _moguet_status OUTPUT_VARIABLE _moguet_stdout ERROR_VARIABLE _moguet_stderr)
file(WRITE "${_moguet_negative_state_dir}/slice7c-late.diagnostic.txt" "${_moguet_stdout}${_moguet_stderr}")
if("${_moguet_status}" STREQUAL "0" OR NOT "${_moguet_stdout}${_moguet_stderr}" MATCHES "redefinition")
    message(FATAL_ERROR "S7-C late failed expected rejection: ${_moguet_status}\n${_moguet_stdout}${_moguet_stderr}")
endif()
math(EXPR _moguet_s7c_count "${_moguet_s7c_count} + 1")
message(STATUS "Slice 7-C narrow baselines=2, positive=1, expected diagnostics=${_moguet_s7c_count}")
